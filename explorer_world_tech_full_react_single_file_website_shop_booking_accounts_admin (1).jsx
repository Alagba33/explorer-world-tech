...
const SERVICES = [
  { id: "s1", name: "Phone Screen Replacement", price: 18000 },
  { id: "s2", name: "Battery Replacement", price: 12000 },
  { id: "s3", name: "Charging Port Repair", price: 10000 },
  { id: "s4", name: "Water Damage Treatment", price: 15000 },
  { id: "s5", name: "Software/OS Update", price: 5000 },
  { id: "s6", name: "General Checkup & Diagnostics", price: 3000 },
  { id: "s7", name: "Laptop Keyboard Replacement", price: 20000 },
  { id: "s8", name: "Laptop Screen Repair", price: 45000 },
];
...
// ---- Payments: Paystack & Flutterwave helpers ----
const PAYSTACK_KEY = "pk_test_your_paystack_key"; // TODO: replace with your real Paystack public key
const FLUTTERWAVE_KEY = "FLWPUBK_TEST-your_flutterwave_key"; // TODO: replace with your real Flutterwave public key
...
async function payWithPaystack(amountNaira, email, metadata = {}) {
  await loadScript("https://js.paystack.co/v1/inline.js");
  return new Promise((resolve, reject) => {
    if (!window.PaystackPop) return reject(new Error("Paystack SDK not available"));
...
async function payWithFlutterwave(amountNaira, email, metadata = {}) {
  await loadScript("https://checkout.flutterwave.com/v3.js");
  return new Promise((resolve, reject) => {
    if (!window.FlutterwaveCheckout) return reject(new Error("Flutterwave SDK not available"));
    window.FlutterwaveCheckout({
      ...
      customer: { email, name: metadata.customerName || "" },
      meta: metadata,
      callback: function (data) {
        resolve({ success: true, reference: data.tx_ref, data });
      },
      onclose: function () {
        resolve({ success: false, closed: true });
      },
...
async function chooseProviderAndPay(amountNaira, email, metadata = {}) {
  const answer = prompt("Pay with: type 'paystack' or 'flutterwave' (or Cancel)");
  const choice = (answer || "").trim().toLowerCase();
  if (choice === "paystack") return payWithPaystack(amountNaira, email, metadata);
  if (choice === "flutterwave") return payWithFlutterwave(amountNaira, email, metadata);
...
export default function ExplorerWorldTechApp() {
  ...
  const checkout = async () => {
    ...
    try {
      const payment = await chooseProviderAndPay(discountedTotal, user.email, {
        purpose: "order",
        orderItems: cart.map((i) => ({ id: i.id, name: i.name, qty: i.qty, price: i.price })),
        userId: user.id,
      ...
      const order = { ...orderPreview, paymentRef: payment.reference };
      setOrders((o) => [order, ...o]);
      setCart([]);
  ...
  const payAndBook = async () => {
    ...
    try {
      const payment = await chooseProviderAndPay(amount, user.email, {
      ...
      pushNotif("Appointment booked & paid", `${service.name} on ${bookingForm.date} @ ${bookingForm.time}`);
      setSection("home");
      setBookingForm({ serviceId: "s1", date: "", time: "", note: "", photo: "", referee: "" });
    } catch (err) {
      alert("Payment error: " + err.message);
            ...
            <Card>
              <div className="grid gap-3">
                ...
                <div className="mt-3 flex gap-2">
                  ...
                  <Button variant="outline" onClick={payAndBook}>Pay & Book</Button>
